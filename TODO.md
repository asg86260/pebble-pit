...
Seam 2, `upgrades.js` is four modules, is BUILT (2026-09-15): `levels.js`
(what every ladder and station is worth), `staffing.js` (the crew's counts),
`words.js` (how a board says a number), `upgrades.js` (rows and buying).
No sim file imports the shop any more, but the ring does not shrink yet:
...
its legs, a wizard's strength is its bolt). The favor list under a pot names
the trades whose station stands and no others. Built as designed, with the
owner's amendment (unbuilt trades off the list, not dimmed), the bracing
tonic red, and strength on the farm as more crop a cut. One thing the design
did not foresee: a quarter more of one grain is one grain, so a whole count
under the strong brew rounds up (`stronger`). Check: test/three-brews.test.mjs.
...
blow -- read straight off the body, so a tile never moves when the site does
not. Not built from the design: the body stepping along the glyph's foot as
the yard's steps along the patch.
...
## 4. The scrubber balloon -- CUT (2026-09-22)

The balloons are gone: the air filter's dial stands where they were moored,
and the two overlapped. DESIGN.md, "The balloons are cut". The clog and the
fleet that were still to do here go with them.
