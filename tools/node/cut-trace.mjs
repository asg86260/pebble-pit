...
const who = () => S.workers.filter(w => w.type === 'quarrier').map(w =>
  `${w.goal}${w.resting ? '~' : ''}@${col(w)} cell=${w.cell} run=${(w.run || []).join('/')} in=${Math.round((w.next || 0) - yard.clock.now())}${w.route ? ' R:' + (w.route[0] && w.route[0].along ? w.route[0].along.key : '?') + '@' + Math.round(w.y) : ''}${w.wanting ? ' K' : ''}${w.trained ? ' T' : ''}${w.holding ? ' H' : ''}`).join(' | ') + ' muck=' + smog.quarryMuck();
