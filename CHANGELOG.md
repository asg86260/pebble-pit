...
- The balloons ride the clouds: each moors at its own post beside the air filter, its rider walks straight there, and up top it travels from cloud to cloud between the layers, drawing a stream down out of each one and bringing the catch home to throw on the filter's heap; they are bigger, shaped like real hot-air balloons, and each wears its own design (src/scenes.js `balloon`, `moored`; test/balloon.test.mjs).
