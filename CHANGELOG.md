...
- The belt ends in a little ramp that flicks its load up and out over the hole in a fan, the front of the heap sloping down to it, instead of dropping it in hard vertical sheets (src/scenes.js `beltdrop`).
...
- The balloons ride the clouds: each moors at its own post beside the air filter, its rider walks straight there, and up top it travels from cloud to cloud between the layers, drawing each one in and bringing the catch home to throw on the filter's heap (src/scenes.js `balloon`, `moored`; test/balloon.test.mjs).
