...
**New this release**
- The belt pours its load off the head in a fan, the top of the heap flying furthest, instead of dropping it into the hole in hard vertical sheets (src/scenes.js `beltdrop`).
...
- A crew switch in the top-right corner, beside fullscreen, steps the workers through show, fade and hide, so a crowded late yard's buildings can be seen (src/selftest/touch.js).
