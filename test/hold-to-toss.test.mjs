// Hold to toss: a button held down on dust sweeps it up and throws it at the
// hole by itself, as far as the reach ladder lets it (`holdToToss` in game.js,
// `tossAtHole` in hands.js).
...
group('a held hand throws the heap under it into the hole once hold to toss is bought', async () => {
  ...
  return [
    ...
    ok(grainsBetween(near - P * 2, near + P * 2) === 0, 'and the ground under the hand is bare',
       `${grainsBetween(near - P * 2, near + P * 2)} left`)
  ];
});
