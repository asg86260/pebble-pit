...
group('the ram does not strike a rock that is still coming down', async () => {
  ...
  // And once it is down, the ram goes back to work. `ready` also waits for
  // room on the rock's heap, and the heap here stands over its limit by an
  // amount that depends on how the run went; how long eight haulers take to
  // shift it is a fact about the heap, not about the ram's rule, so the heap
  // is taken away and the rule is asked on its own.
  window.__clearFloor();
