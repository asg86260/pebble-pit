...
import { parkX } from '../src/crew/lifts.js';
...
group('a forklift is bought off the bench and rolls off the stand by itself', async () => {
  window.__reset();
  openSites();
  window.__fullSites();
  window.__crew(0, 2);
  ...
  return [
    ...
    ok(at && Math.abs(at.x - parkX(0)) < 1, 'standing at its slot on the stand', `${at?.x} against ${parkX(0)}`),
...
group('forklifts and haulers share the piles and never go for the same column', async () => {
  lip(3);
  buyNow('driver');
  buyNow('driver');
  window.__clearFloor();
  ...
  for (let i = 0; i < 30 * 6; i++) {
    ...
    const claims = [...S.workers.filter(w => w.type === 'hauler'), ...S.lifts]
      .map(w => w.claim).filter(c => c >= 0);
    if (new Set(claims).size !== claims.length) clash++;
  }
  return [ok(clash === 0, 'no two of them ever hold the same column', `${clash} frames with a clash`)];
});

group('with the yard clean, a forklift parks at its slot and smokes nothing', async () => {
  lip();
  buyNow('driver');
  window.__clearFloor();
  heap();
  ...
  const laden1 = soot();
  window.__clearFloor();
  runUntil(() => lift().goal === 'idle' && Math.abs(lift().x - parkX(0)) < 1, 60);
  const parked = lift();
  const idle0 = soot();
  run(8);
  return [
    ok(laden1 > laden0, 'a laden drive puts soot up', `${laden0} -> ${laden1}`),
    ok(Math.abs(parked.x - parkX(0)) < 1, 'and with nothing to fetch it goes back to its slot',
       `${Math.round(parked.x)} against ${parkX(0)}`),
    // No more than there was: what is up there drifts off on its own.
    ok(soot() <= idle0, 'and puts nothing up standing there', `${idle0} -> ${soot()}`),
    ok(!parked.brk && !parked.inside, 'and takes no break and goes in nowhere')
  ];
});
