...
// comparisons twice a second is nothing; at sixty hertz it would be a thing
// somebody had to measure.
export const NOTICE_TICK_S = 0.5;

// --- the thresholds -----------------------------------------------------------
// Placeholders that want a played yard rather than an argument. Tables rather
// than named constants: a new rung is a number in a list, and the notice for
// it is built from the list, so the count on the board and the thresholds
// cannot drift apart. Not `TUNABLE` rows: a slider edits one number, and one
// number here is a rung of a ladder whose shape is the point.
...
// What counts as a big hand at the table, either way.
...
// enough to catch out of the corner of the eye and then read without hurrying
// (four seconds went unnoticed under a busy yard); short enough that a rock
...
// A panel on two posts between the work bench and the houses: furniture, not a
// building. Wide enough for the sheets on it to be legible at the yard's
// ordinary zoom and no wider, since the crew have to get past it. Thirteen is
// what three three-cell sheets with a cell of board between and around them
// come to (SHEETS in render/noticeboard.js): the panel is sized off its
// contents, not the contents squeezed to a panel.
...
// the clouds' whites and a step under. Each sheet takes one off its own index,
// because paper on a board is not one white (render/noticeboard.js).
