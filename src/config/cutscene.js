...
// A shield is tall -- the arch's crown is forty-odd cells up -- and wide --
// the dome's ring is eighty-odd cells across -- and what is watched is the
// rock meeting its top, so the pull-in is measured against the thing rather
// than fixed: the span, the rock over it and a little sky fill this much of
// the frame's height, the span this much of its width, and the zoom is
// whatever makes the tighter of the two so, never closer than the step above.
// A desk's window is bound by its height, a phone stood upright by its width.
