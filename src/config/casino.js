...
// --- the drop -----------------------------------------------------------------
// A plinko board on the casino's roof, and the second way a pot gets settled.
// The wheel is even money and nothing else -- every hand has two shapes -- and
// what the board adds is *spread*: one rock carries the whole pot down ten rows
// of pegs, and the bin it lands in is what the pot is multiplied by. See
// DESIGN.md, "The drop".
//
...
// the way the wheel's odds are its slices, and the bins say what they pay.
export const PLINKO_ROWS = 10;
// What each bin pays, edge to edge. Written out rather than worked from a
// formula so the numbers can be read off the board -- and they are chosen so
// that, weighted by how often a rock reaches each one (1, 10, 45, 120, 210, 252
// in 1,024, and back), the table pays exactly one. The drop is fair to the
// grain, the same as the wheel; the house's whole edge is that a fair bet taken
// for ever ends at nothing. `test/plinko.test.mjs` asserts the sum, so a bin
// cannot be tuned on its own.
export const PLINKO_BINS = [39, 5, 3, 1, 0.5, 0.5, 0.5, 1, 3, 5, 39];
// How long the rock takes to get to the bottom, hopper to bin: the wheel's own
// two and a half seconds and a bit, because it is the same kind of moment --
// the one thing in this game you are meant to sit and watch.
export const PLINKO_MS = 2700;
// And how long the rock takes to be winched back up to the hopper afterward.
// Nothing pops here either: the rock sits in its bin and climbs the tower's
// side, and the drop is shut until it is home.
export const PLINKO_HOME_MS = 1400;
export const PLINKO_KNOCK = 9;        // what a landing does to the view
export const PLINKO_EDGE_KNOCK = 18;  // and an outer bin, which is the big one
// The board, in cells. Bins are four cells apart so a three-cell glyph fits in
// each, and the peg rows two apart, so a rock a cell either side of its column
// lands dead in the middle of a bin: the board's width is the bins times the
// pitch, and that is why it is forty-four rather than a round number.
export const PLINKO_PITCH = 4;         // cells between bin centers
export const PLINKO_ROW_H = 2;         // cells between peg rows
export const PLINKO_SIGN_H = 9;        // the sign across the top: five-cell letters and two of air
export const PLINKO_HOPPER_H = 4;      // where the rock waits
export const PLINKO_BIN_H = 4;         // a white slot the rock lands in
// What a bin pays is written under its slot, white on the black of the tower:
// a glyph in the slot itself touched the black divider beside it and the two
// fused, three half-marks in a row into one bar. Tall enough for two digits
// stacked with a cell of air between and above.
export const PLINKO_LABEL_H = 12;
