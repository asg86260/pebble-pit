...
const TESTS = [
  ...
  ['a ripe bed shows its spore before it is cut', async () => {
    ...
    quickCrew();
    window.__clearFloor();
    // The beds an earlier check left standing ripe, so this one waits for a
    // spore that grows while it is watching. A bed nobody is working keeps its
    // tone for ever, and a check that picked one of those would sit waiting for
    // a farmhand at the other end of the farm to come and cut it.
    const standing = state().bedTone.map(t => t > 0);
    const fresh = tones => tones.findIndex((t, k) => t > 0 && !standing[k]);
    ...
    for (let i = 0; i < 3000 && !ripe; i++) {
      ...
      ripe = fresh(state().bedTone) >= 0;
    }
    const showing = state();
    const i = fresh(showing.bedTone);
