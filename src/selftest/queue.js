...
// A clock's `m:ss` as seconds, wherever it sits in the line's text; NaN for
// a line with no clock.
const secs = text => { const m = /(\d+):(\d\d)/.exec(text); return m ? +m[1] * 60 + +m[2] : NaN; };
...
export const TESTS = [
  ['the queue card is absent until something is building, then lists the line', async () => {
    ...
    return [
      ...
      // Two clocks. The front's is a figure, or `queued` while the builder is
      // still walking over; the waiting one's is always a figure behind its
      // place word, and more than the front's when the front has one, since
      // it counts both.
      ok(two.clocks.length === 2 && secs(two.clocks[1]) > 0 && /^next /.test(two.clocks[1])
         && (two.clocks[0] === 'queued' || secs(two.clocks[1]) > secs(two.clocks[0]) && secs(two.clocks[0]) > 0),
