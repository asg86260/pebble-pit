...
export const TESTS = [
  ['one finger on the yard scrolls it, the platform\'s way, and a finger on dust sweeps', async () => {
    ...
    let shutHeld = null, shutAfter = null;
    if (spot != null) {
      ...
      shutHeld = scroller().style.overflowX;                  // ...and the scroller is shut under it, so a platform that ignored the refusal has nothing to pan
      const mid = state();
      ...
      shutAfter = scroller().style.overflowX;
      await frames(1);
    ...
    return [
      ...
      ok(shutHeld === 'hidden' && shutAfter === '', 'the scroller is shut for the length of a sweep and open once the finger lifts', `held ${shutHeld}, after ${shutAfter}`),
