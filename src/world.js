...
import { P, CELL, SKY, SKY_UP, SKY_R, TO_BENCH, TO_QUARRY, TO_LEDGE, GROUND_LEFT,
        ...
        BOARD_COLS, BOARD_ROWS, TRAY_H, CASINO_SIGN_H,
...
import { S, floor, pit, bench, quarry, farm, apothecary, sky, school, casino, scrub, table, board, tower, outhouse, shack } from './state.js';
...
export function resize(after) {
  ...
  // The casino's two plots of sand: the tray on its roof, where the stake
  // stands, and the board on its face, where the stake ends up. Both are the
  // building's inner width, a wall in from each side; the tray stands on the
  // roof and the board hangs under the sign. See casino.js.
  table.x = casino.x + P;
  table.cols = BOARD_COLS;
  table.rows = TRAY_H;
  table.y = casino.y - TRAY_H * P;
  board.x = casino.x + P;
  board.cols = BOARD_COLS;
  board.rows = BOARD_ROWS;
  board.y = casino.y + CASINO_SIGN_H * P;
