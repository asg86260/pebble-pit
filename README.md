...
- Drag along the ground to sweep dust onto the cursor, then flick and let go to throw it. Anything past the ledge falls into the pit on the right.
...
- Workers are plain squares: **miners** (hollow centre) orbit the boulder and chip at it, **haulers** (outlined) walk the ground, scoop dust, and tip it over the ledge. Both start slow and carrying one pixel; their upgrades only appear once you own one.
